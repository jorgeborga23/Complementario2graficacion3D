...
def plotTriangleLine(x,y,z,xc,yc,zc,pointx,pointy):
    #   Coordenadas locales
    x=[40,30,80,pointx]
    y=[60,10,60,pointy]
    z=[0,0,0,0]
...
entradax=input("Coordenada x del Hitpointx:  (Escriba ESC para salir)")

if(entradax == 'ESC'):
    sys.exit()
#pointx=int(hitpointx)
pointx=float(entradax)
    
entraday=input("Coordenada y del Hitpointy: (Escriba ESC para salir)")
    
#pointy=int(hitpointy)
if(entraday == 'ESC'):
    sys.exit()
pointy=float(entraday)
#Mandamos los valores al metodo que dibuja el triangulo
plotTriangleLine(x,y,z,xc,yc,zc,pointx,pointy)
